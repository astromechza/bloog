...
fn pulldown_parser(content: &str) -> (Arc<Mutex<Option<String>>>, Parser<BrokenLinkTracker>) {
    ...
    let parser = Parser::new_with_broken_link_callback(
        content,
        pulldown_cmark::Options::ENABLE_STRIKETHROUGH | pulldown_cmark::Options::ENABLE_DEFINITION_LIST | 
            pulldown_cmark::Options::ENABLE_FOOTNOTES | pulldown_cmark::Options::ENABLE_TABLES,
        Some(BrokenLinkTracker{tracker: error_capture.clone()}),
    );
            ...
            if !valid_links.is_empty() {
                if let Some((link_type, dest_url)) = match &event {
                    ...
                    if let Ok(mut locked) = error_capture.lock() {
                        locked.replace(format!("{} '{}' references to a path which does not exist", link_type, dest_url));
                    }
                }
            }
        });
