...
async fn get_image_handler(
    ...
    if let Some(image) = store.get_image_raw(&img).await.map_resp_err(&HtmxContext::try_from(&headers).ok())? {
        let mut hm = HeaderMap::new();
